...
import { CheckCircle2, TrendingUp, Handshake, BadgeEuro, Search, Calculator, Smartphone, Star, Quote, Calendar } from "lucide-react";
...
export default async function Home() {
  ...
  return (
    <>
      ...
      {/* Blog Section */}
      <section className="py-16 sm:py-24 bg-[#0f172a] border-t border-white/5 relative overflow-hidden">
        {/* Decorative Background */}
        <div className="absolute top-1/2 right-1/4 w-80 h-80 bg-[#FBBF24]/5 rounded-full mix-blend-screen filter blur-3xl opacity-30 z-0"></div>
        
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            <div className="inline-block bg-[#FBBF24]/10 border border-[#FBBF24]/30 text-[#FBBF24] font-bold px-4 py-2 rounded-full mb-6 backdrop-blur-sm shadow-[0_0_15px_rgba(251,191,36,0.2)] text-sm">
              📰 Actualidad y Consejos
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold font-heading text-white mb-6">
              El Blog de <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FBBF24] to-yellow-200">Tu Asesor</span>
            </h2>
            <p className="text-lg text-slate-300 max-w-2xl mx-auto font-light font-sans">
              Descubre las últimas tendencias del mercado inmobiliario en Sevilla, guías prácticas para vender al mejor precio y consejos financieros clave.
            </p>
          </div>

          {posts.length === 0 ? (
            <div className="text-center py-12 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 max-w-2xl mx-auto">
              <p className="text-slate-400">Pronto publicaremos nuevos artículos. ¡Mantente atento!</p>
            </div>
          ) : (
                ...
                <Link key={post.id} href={`/blog/${post.slug}`} className="group block">
                  <article className="h-full glass-effect bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-xl transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(251,191,36,0.15)] flex flex-col">
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <h3 className="text-xl font-bold font-heading mb-3 text-white group-hover:text-[#FBBF24] transition-colors line-clamp-2">
                        {post.title}
                      </h3>
                      {post.excerpt && (
                        <p className="text-slate-300 line-clamp-3 mb-6 font-light text-sm font-sans">
                          {post.excerpt}
                        </p>
                      )}
          ...
          {posts.length > 0 && (
            <div className="text-center mt-12">
              <Link href="/blog" className="btn btn-outline border-[#FBBF24] text-[#FBBF24] hover:bg-[#FBBF24] hover:text-[#0f172a] px-8 py-3 rounded-full font-bold transition-all inline-block">
